...
void displayHighscores() {
    ...
    if (currentState == HIGHSCORE_SCREEN) {
        glFlush();
        glutSwapBuffers();
    }
...
void displayGameOver() {
    ...
    */
    ...
    drawText(-0.29f, -0.4f, "YOUR SCORE", GLUT_BITMAP_HELVETICA_18);

    // Display the player's score
    char scoreText[50];
    snprintf(scoreText, sizeof(scoreText), "%d", highscore);  // Assuming highscore holds the player's score
    drawText(-0.05f, -0.5f, scoreText, GLUT_BITMAP_HELVETICA_18);

    // Restart option
    glColor3f(1.0f, 1.0f, 0.0f);  // Neon Yellow for options
    drawText(-0.25f, -0.7f, "Restart Game", GLUT_BITMAP_HELVETICA_18);

    drawText(-0.43f, -0.85f, "Press Enter to restart the game.", GLUT_BITMAP_HELVETICA_12);  // Escape to return
