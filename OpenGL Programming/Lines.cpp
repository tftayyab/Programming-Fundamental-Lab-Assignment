...
char filepath[20] = "Highscores.txt";
...
void snake_move(int deltaX, int deltaY) {
    int i;
...
void displayHighscores() {
    ...
    if (!(currentState == GAME_OVER)) {
        drawText(-0.45f, -0.7f, "Press Escape to return to menu.", GLUT_BITMAP_HELVETICA_12);
    }
...
void displayGameOver() {
    ...
    char scoreText[50];
    snprintf(scoreText, sizeof(scoreText), "%d", highscore);
...
void handleMenuInput(unsigned char key, int x, int y) {
    ...
    if (currentState == INTRO_MENU) {
        ...
        if (tolower(key) == 'w') {
            currentOption = (currentOption - 1 + 2) % 2; // Move up 
        }
        else if (tolower(key) == 's') {
            currentOption = (currentOption + 1) % 2; // Move down
        }
    }
