...
void displayGameOver() {
    ...
    drawText(-0.2, 0.5, "GAME OVER", GLUT_BITMAP_TIMES_ROMAN_24);  // Adjust position as needed

    // Draw the options below "Game Over"
    drawText(-0.3, 0.2, currentOption == 0 ? "> Restart Game" : "Restart Game", GLUT_BITMAP_HELVETICA_18);
    drawText(-0.3, 0.0, currentOption == 1 ? "> Back to Menu" : "Back to Menu", GLUT_BITMAP_HELVETICA_18);
...
void displaychecker() {
    ...
    glutSwapBuffers();  // Swap buffers to display the content
    glFlush();  // Make sure the content is rendered immediately
}
...
void endGame() {
    ...
    terminate = 1;
    SnakegameTerminator();
    glutDisplayFunc(displaychecker);  // Set display function to show Game Over screen
}
...
void handleMenuInput(unsigned char key, int x, int y) {
    ...
    if (currentState == INTRO_MENU) {
        ...
        if (key == '\r' || key == '\n') { // Enter key
            ...
            else if (currentOption == 1) { // back to menu
                currentState = INTRO_MENU;  // Set state to intro menu
                glutDisplayFunc(displaychecker);
            }
        }
    }
