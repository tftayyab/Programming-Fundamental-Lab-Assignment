...
void appendHighscore(const char* name, int score) {
    FILE* file = fopen("E:\Download\Computer-Graphics-main\Computer-Graphics-main\OpenGL Programming\highscores.txt", "a");
    if (file) {
        fprintf(file, "%s %d\n", name, score);
        fclose(file);
    }
...
void updateHighScores(int playerScore, char* playerName) {
    // Load current high scores
    FILE* file = fopen("E:\Download\Computer-Graphics-main\Computer-Graphics-main\OpenGL Programming\highscores.txt", "r+");
    if (file == NULL) {
    ...
    int score, i = 0;
    char name[30];
    int highScores[5];  // Array to store high scores (for example)
    char highScoreNames[5][30];  // Array to store names associated with high scores

    // Read current high scores
    while (fscanf(file, "%s %d", highScoreNames[i], &highScores[i]) == 2) {
        i++;
    }

    // Check if player's score should be inserted
    int position = -1;
    for (int j = 0; j < 5; j++) {
    ...
    // If the player's score is in the top 5, update the list
    if (position != -1) {
        for (int j = 4; j > position; j--) {
            highScores[j] = highScores[j - 1];
            strcpy(highScoreNames[j], highScoreNames[j - 1]);
        }
        highScores[position] = playerScore;
        strcpy(highScoreNames[position], playerName);

        // Reopen the file in write mode to update it
        freopen("E:\Download\Computer-Graphics-main\Computer-Graphics-main\OpenGL Programming\highscores.txt", "w", file);

        // Write the updated high scores
        for (int i = 0; i < 5; i++) {
...
void endGame() {
    updateHighScores(playerScore, playerName);  // Update high scores in the file

    printf("Game Over! Your score: %d\n", playerScore);  // Print the final score in the console
...
void displayFunc() {
    ...
    {
        int i;
        highscore = (sanp.length - 1) * 100; // Changed snake to sanp
        for (i = 0; i < 5; i++) {
            int digit = highscore - ((highscore / 10) * 10);
            int tileId = 15 + digit;
            highscore /= 10;
            glBegin(GL_QUADS);
            glTexCoord2f(tiles[tileId].x1 / (float)snakeTileSet.width, (tiles[tileId].y1) / (float)snakeTileSet.height);
...
void readHighscores(char highscores[5][100]) {
    FILE* file = fopen("E:\Download\Computer-Graphics-main\Computer-Graphics-main\OpenGL Programming\highscores.txt", "r");
    if (!file) {
        for (int i = 0; i < 5; i++) {
            strcpy(highscores[i], "No Data");
        }
        return;
    }
    ...
    int score;
    int i = 0;
    while (fscanf(file, "%s %d", name, &score) != EOF && i < 5) {
        sprintf(highscores[i], "%s: %d", name, score);
        i++;
    }
...
void displayHighscores() {
    char highscores[5][100];
    readHighscores(highscores);

    drawText(-0.3, 0.7, "Highscores", GLUT_BITMAP_TIMES_ROMAN_24);

    float yPos = 0.5;
    for (int i = 0; i < 5; i++) {
        drawText(-0.5, yPos, highscores[i], GLUT_BITMAP_HELVETICA_18);
        yPos -= 0.2;
    }
...
void display() {
    ...
    if (currentState == INTRO_MENU) {
        ...
        drawText(-0.5, -0.1, playerName, GLUT_BITMAP_HELVETICA_18);
    }
...
void handleMenuInput(unsigned char key, int x, int y) {
    ...
    if (currentState == INTRO_MENU) {
        ...
        if (key == 8) { // Backspace to delete the last character in the name
            int len = strlen(playerName);
            if (len > 0) {
                playerName[len - 1] = '\0';
            }
        }
        else if (key == '\r' || key == '\n') { // Enter key to start the game after entering name
            if (strlen(playerName) > 0) {
                startGame();
            }
        }
        else if (isprint(key)) { // Printable characters to add to the player's name
            int len = strlen(playerName);
            if (len < 49) { // Limit name length to 50 characters
                playerName[len] = key;
                playerName[len + 1] = '\0';
            }
        }
