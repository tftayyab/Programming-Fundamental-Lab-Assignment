...
void displayGameOver() {
    ...
    drawText(-0.35f, 0.6f, "GAME OVER", GLUT_BITMAP_TIMES_ROMAN_24);

    // Highscores Section
    glColor3f(1.0f, 1.0f, 0.0f);  // Neon Yellow for the "Highscores" label
    drawText(-0.3f, 0.45f, "Highscores", GLUT_BITMAP_TIMES_ROMAN_24);

    // Load and sort high scores
    resetHighScore();
    storescore();
    loadHighScores();
    qsort(highScores, totalScores, sizeof(HighScore), compareScores);

    // Display top 5 high scores
    glColor3f(1.0f, 0.0f, 0.0f);  // Bright Red for scores
    for (int i = 0; i < 5; i++) {
        char highScoreText[50];
        snprintf(highScoreText, sizeof(highScoreText), "%d. %s - %d", i + 1, highScores[i].name, highScores[i].score);
        drawText(-0.4f, 0.3f - i * 0.1f, highScoreText, GLUT_BITMAP_HELVETICA_18);
    }

    // "Your Score Is"
    glColor3f(1.0f, 0.0f, 0.0f);  // Bright Red for text
    drawText(-0.29f, -0.3f, "YOUR SCORE", GLUT_BITMAP_HELVETICA_18);

    // Display the player's score
    char scoreText[50];
    snprintf(scoreText, sizeof(scoreText), "%d", highscore);  // Assuming highscore holds the player's score
    drawText(-0.05f, -0.4f, scoreText, GLUT_BITMAP_HELVETICA_18);

    // Restart option
    glColor3f(1.0f, 1.0f, 0.0f);  // Neon Yellow for options
    drawText(-0.25f, -0.6f, "Restart Game", GLUT_BITMAP_HELVETICA_18);

    drawText(-0.43f, -0.75f, "Press Enter to restart the game.", GLUT_BITMAP_HELVETICA_12);  // Escape to return
...
void idleFunc() {
    ...
    if (isGameOver == 1) {
        ...
        usleep(2000000 / (7 + (sanp.length / (float)foods) * 10.0));
